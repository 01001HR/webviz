...
export function makeCommand<T>(
  ...
  const cmd = ({
    ...
    if (getHitmapId) {
      console.warn(
        `"getHitmapId" is deprecated. Check "${name}" to use "enableHitmap" to turn on the default hitmap mapping or set the "getHitmapProps" and "getObjectFromHitmapId" props explicitly. `
      );
